...
class BytenutRenewal:
    ...
    def try_extend_and_verify(self, sb, server_id, old_expiry):
        ...
        for _ in range(6):
            if self.manual_verification_present(sb):
                self.mark_failed(
                    "ByteNut requested manual OTP/email verification"
                )
    ...
    def run(self):
        ...
        if not accounts:
            self.log("❌ 无账号")
            self.mark_failed("No accounts found in BYTENUT")
            sys.exit(1)
            ...
            with SB(
                ...
                try:
                        ...
                        if can_extend:
                            ...
                            if result is True:
                                ...
                                self.send_tg(
                                    ...
                                    user, server_id, "offline", expiry_str,
                                    "ByteNut is asking for an OTP/email code. Renew this server manually.",
                                    screenshot=self.shot(
                                        sb, f"manual_verification_{idx}.png"))
                    ...
                    if result is True:
                        ...
                        self.send_tg(
                            "\u26a0\ufe0f", "Manual verification required",
                            user, server_id, state, expiry_str,
                            "ByteNut is asking for an OTP/email code. Renew this server manually.",
                            screenshot=self.shot(
                                sb, f"manual_verification_{idx}.png"))
                    elif result == "cooldown":
        ...
        if self.had_failure:
            self.log("❌ One or more accounts failed; exiting with status 1")
